...
def plan_path(
    ...
    # Allow blocked start/goal cells as one-cell exceptions only.
    # This lets the robot start/end inside inflated obstacle clearance,
    # while keeping all other blocked cells illegal during planning.
    occ_for_planning = occ.copy()
    if occ_for_planning[sy, sx]:
        print("NOTE: Start is inside an inflated obstacle; allowing start cell only.")
        occ_for_planning[sy, sx] = False
    if occ_for_planning[gy, gx]:
        print("NOTE: Goal is inside an inflated obstacle; allowing goal cell only.")
        occ_for_planning[gy, gx] = False

    # Wavefront + path
    cost = wavefront_cost(occ_for_planning, (gx, gy))
    ...
    if show_plot:
        ...
        plt.imshow(~occ_for_planning, origin="lower")  # free bright, blocked dark
        plt.plot(xs, ys)
